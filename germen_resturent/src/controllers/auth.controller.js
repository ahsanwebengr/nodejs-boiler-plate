...
const loginUser = async (req, res) => {
    ...
    try {
        ...
        if (isPasswordValid) {
            ...
            res.status(401).json({ error: "Invalid password", status: false });
        }
    } catch (err) {
        console.error(err);
        res.status(500).json({ error: "Internal Server Error", status: false });
...
const forgotPassword = async (req, res) => {
    try {
        ...
        if (!userEmail) {
            res.status(404).json({ error: 'This email does not exist', status: false });
        }
    ...
    `,
        ...
        res.status(200).json({ message: 'OTP sent to your email for password reset', status: 'Success' });
    } catch (err) {
        console.log(err);
